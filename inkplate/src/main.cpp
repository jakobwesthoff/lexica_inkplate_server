...
#error "Wrong board selection, please select Inkplate Color in the boards menu."
#endif
...
const char *request_url = "http://192.168.178.3:9000/lexica/inkplate";

Inkplate display;

size_t http_request(const char *url, byte *buffer, size_t buffer_size);
...
void setup()
{
  ...
  if (buffer == nullptr)
  {
    ...
    display.setCursor(0, E_INK_HEIGHT - 50);
    display.println("Could not allocate memory!");
    display.display();
    delay(5000);
    ESP.restart();
  }

  size_t received = http_request(request_url, buffer, buffer_size);
  ...
  else
  {
    display.setTextColor(7, 0);
    display.setCursor(0, E_INK_HEIGHT - 50);
    display.println("Received image incomplete!");
    display.display();
    delay(5000);
    log_d("Received image incomplete");
    log_d("Going to sleep for 5 minutes");
    goto_sleep(300 * uS_TO_S_FACTOR);
  }
